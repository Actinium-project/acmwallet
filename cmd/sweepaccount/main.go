...
var (
	walletDataDirectory = acmutil.AppDataDir("acmwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
func makeInputSource(outputs []acmjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue acmutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]acmutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := acmutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(acmutil.Amount) (acmutil.Amount, []*wire.TxIn, []acmutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	sourceOutputs := make(map[string][]acmjson.ListUnspentResult)
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := acmutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount acmutil.Amount) bool {
	return amount >= 0 && amount <= acmutil.MaxSatoshi
}

func parseOutPoint(input *acmjson.ListUnspentResult) (wire.OutPoint, error) {
