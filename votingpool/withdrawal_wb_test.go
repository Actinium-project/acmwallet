...
func TestOutputSplittingNotEnoughInputs(t *testing.T) {
	...
	output1Amount := acmutil.Amount(2)
	output2Amount := acmutil.Amount(3)
...
func TestOutputSplittingOversizeTx(t *testing.T) {
	...
	requestAmount := acmutil.Amount(5)
	bigInput := int64(3)
	smallInput := int64(2)
	...
	if tx1.outputs[0].amount != acmutil.Amount(bigInput) {
	...
	if tx2.outputs[0].amount != acmutil.Amount(smallInput) {
...
func TestFulfillRequestsNoSatisfiableOutputs(t *testing.T) {
	...
	request := TstNewOutputRequest(
		t, 1, "MRJkUjbhhmiTWYaNZVbNFrgxYDmoJzv4M6", acmutil.Amount(3e6), pool.Manager().ChainParams())
	changeStart := TstNewChangeAddress(t, pool, seriesID, 0)
...
func TestFulfillRequestsNotEnoughCreditsForAllRequests(t *testing.T) {
	...
	out1 := TstNewOutputRequest(
		t, 1, "MRJkUjbhhmiTWYaNZVbNFrgxYDmoJzv4M6", acmutil.Amount(3e6), net)
	out2 := TstNewOutputRequest(
		t, 2, "MDN7A7WaVBhcE1r7qF6fermgULrn7cP4wX", acmutil.Amount(2e6), net)
	out3 := TstNewOutputRequest(
		t, 3, "M965iBTPbU5BAWm2VDdzmWMGMM7w5iLe3p", acmutil.Amount(5e6), net)
...
func TestRollbackLastOutputMultipleInputsRolledBack(t *testing.T) {
	...
	for i, amount := range []acmutil.Amount{4, 3, 2} {
...
func TestRollBackLastOutputInsufficientOutputs(t *testing.T) {
	...
	output := &WithdrawalOutput{request: TstNewOutputRequest(
		t, 1, "MRJkUjbhhmiTWYaNZVbNFrgxYDmoJzv4M6", acmutil.Amount(3), &chaincfg.MainNetParams)}
...
func TestRollbackLastOutputWhenNewOutputAdded(t *testing.T) {
	...
	checkTxChangeAmount(t, firstTx, acmutil.Amount(4))
	...
	checkTxChangeAmount(t, secondTx, acmutil.Amount(3))
}
...
func TestWithdrawalTxAddChange(t *testing.T) {
	...
	tx.calculateFee = TstConstantFee(acmutil.Amount(fee))
...
func TestWithdrawalTxInputTotal(t *testing.T) {
	...
	if tx.inputTotal() != acmutil.Amount(5) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), acmutil.Amount(5))
	}
}
...
func TestWithdrawalTxOutputTotal(t *testing.T) {
	...
	if tx.outputTotal() != acmutil.Amount(4) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), acmutil.Amount(4))
	}
}
...
func TestSignMultiSigUTXOPkScriptNotP2SH(t *testing.T) {
	...
	mgr := pool.Manager()
	tx := createWithdrawalTx(t, dbtx, pool, []int64{4e6}, []int64{})
	addr, _ := acmutil.DecodeAddress("LYseubZ88VVVsY9EvcAWtEQ2nMmcgYz6NX", mgr.ChainParams())
	pubKeyHashPkScript, _ := txscript.PayToAddrScript(addr.(*acmutil.AddressPubKeyHash))
	msgtx := tx.toMsgTx()
...
func TestSignMultiSigUTXORedeemScriptNotFound(t *testing.T) {
	...
	// script.
	addr, _ := acmutil.DecodeAddress("MRJkUjbhhmiTWYaNZVbNFrgxYDmoJzv4M6", mgr.ChainParams())
	if _, err := mgr.Address(addrmgrNs, addr); err == nil {
		t.Fatalf("Address %s found in manager when it shouldn't", addr)
	}
	msgtx := tx.toMsgTx()

	pkScript, _ := txscript.PayToAddrScript(addr.(*acmutil.AddressScriptHash))
...
func TestStoreTransactionsWithChangeOutput(t *testing.T) {
	...
	inputTotal := acmutil.Amount(0)
	for _, debit := range txDetails.Debits {
		inputTotal += debit.Amount
	}
	if inputTotal != acmutil.Amount(5e6) {
		t.Fatalf("Unexpected input amount; got %v, want %v", inputTotal, acmutil.Amount(5e6))
	}
...
func createWithdrawalTxWithStoreCredits(t *testing.T, dbtx walletdb.ReadWriteTx, store *wtxmgr.Store, pool *Pool,
	...
	for i, amount := range outputAmounts {
		request := TstNewOutputRequest(
			t, uint32(i), "MRJkUjbhhmiTWYaNZVbNFrgxYDmoJzv4M6", acmutil.Amount(amount), net)
...
func checkMsgTxOutputs(t *testing.T, msgtx *wire.MsgTx, requests []OutputRequest) {
	...
	for i, request := range requests {
		...
		gotAmount := acmutil.Amount(txOut.Value)
...
func checkTxChangeAmount(t *testing.T, tx *withdrawalTx, amount acmutil.Amount) {
	if !tx.hasChange() {
...
func checkLastOutputWasSplit(t *testing.T, w *withdrawal, tx *withdrawalTx,
	origAmount, newAmount acmutil.Amount) {
