...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="acmwallet RPC"`)
...
// method.  This may be a request that is handled directly by acmwallet, or
// a chain server request that is handled by passing the request down to acmd.
//
...
func (s *Server) handlerClosure(request *acmjson.Request) lazyHandler {
	s.handlerMu.Lock()
...
func sanitizeRequest(r *acmjson.Request) string {
...
// but this is only implemented here to work around an oddity with acmjson,
...
func (s *Server) invalidAuth(req *acmjson.Request) bool {
	cmd, err := acmjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*acmjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := acmjson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := acmjson.MarshalResponse(req.ID, nil, acmjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response: %v", err)
	...
	case "stop":
		stop = true
		res = "acmwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(&req)()
	}

	// Marshal and send.
	mresp, err := acmjson.MarshalResponse(req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response: %v", err)
