...
// request into a marshalable response.  If the error is a *acmjson.RPCError
...
// catch-all error code, acmjson.ErrRPCWallet.
...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by acmwallet due to
...
// unsupported by acmwallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &acmjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by acmwallet",
	}
}
...
// context.
type lazyHandler func() (interface{}, *acmjson.RPCError)
...
func lazyApplyHandler(request *acmjson.Request, w *wallet.Wallet, chainClient chain.Interface) lazyHandler {
	handlerData, ok := rpcHandlers[request.Method]
	if ok && handlerData.handlerWithChain != nil && w != nil && chainClient != nil {
		return func() (interface{}, *acmjson.RPCError) {
			cmd, err := acmjson.UnmarshalCmd(request)
			if err != nil {
				return nil, acmjson.ErrRPCInvalidRequest
			}
...
func makeResponse(id, result interface{}, err error) acmjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return acmjson.Response{
			ID:    idPtr,
			Error: jsonError(err),
		}
	}
...
func jsonError(err error) *acmjson.RPCError {
	if err == nil {
		return nil
	}
	...
	case waddrmgr.ManagerError:
		...
		case waddrmgr.ErrWrongPassphrase:
			code = acmjson.ErrRPCWalletPassphraseIncorrect
		}
	}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*acmutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := acmutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]acmutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.CreateMultisigCmd)
	...
	address, err := acmutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		// above is a valid script, shouldn't happen.
		return nil, err
	}

	return acmjson.CreateMultiSigResult{
...
func dumpPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.DumpPrivKeyCmd)
...
func getAddressesByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetAddressesByAccountCmd)
...
func getBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetBalanceCmd)
...
// information about the current state of acmwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to acmd for all of the information in this command known
	// by them.
...
func decodeAddress(s string, params *chaincfg.Params) (acmutil.Address, error) {
	addr, err := acmutil.DecodeAddress(s, params)
	if err != nil {
...
func getAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetAccountCmd)
...
// appear in the blockchain, or any tx that has arrived in the acmd mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return acmjson.ErrRPCWalletKeypoolRanOut if that happens).
func getAccountAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetAccountAddressCmd)
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetUnconfirmedBalanceCmd)
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ImportPrivKeyCmd)
	...
	wif, err := acmutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCInvalidAddressOrKey,
...
func createNewAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.CreateNewAccountCmd)
...
func renameAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.RenameAccountCmd)
...
func getNewAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetNewAddressCmd)
...
func getRawChangeAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetRawChangeAddressCmd)
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetReceivedByAccountCmd)
...
func getReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetReceivedByAddressCmd)
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.GetTransactionCmd)

	txHash, err := chainhash.NewHashFromStr(cmd.Txid)
	if err != nil {
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCDecodeHexString,
	...
	ret := acmjson.GetTransactionResult{
		TxID:            cmd.Txid,
	...
	var (
		debitTotal  acmutil.Amount
		creditTotal acmutil.Amount // Excludes change
		fee         acmutil.Amount
		feeF64      float64
	)
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var outputTotal acmutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += acmutil.Amount(output.Value)
		}
	...
	if len(details.Debits) == 0 {
		...
		ret.Details = make([]acmjson.GetTransactionDetailsResult, 0, len(details.Credits))
	} else {
		ret.Details = make([]acmjson.GetTransactionDetailsResult, 1, len(details.Credits)+1)

		ret.Details[0] = acmjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Account
			//   Address
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, acmjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Fee
			Account:  accountName,
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*acmjson.HelpCmd)

	// acmd returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to acmd.  Therefore, create a
	...
	// texts in a non-internal acmd package.
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ListAccountsCmd)
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ListReceivedByAccountCmd)
	...
	jsonResults := make([]acmjson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, acmjson.ListReceivedByAccountResult{
...
func listReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ListReceivedByAddressCmd)
	...
	ret := make([]acmjson.ListReceivedByAddressResult, numAddresses, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = acmjson.ListReceivedByAddressResult{
			Address:       address,
...
func listSinceBlock(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*acmjson.ListSinceBlockCmd)
	...
	res := acmjson.ListSinceBlockResult{
...
func listTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ListTransactionsCmd)
...
func listAddressTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ListAddressTransactionsCmd)
...
func listAllTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ListAllTransactionsCmd)
...
func listUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ListUnspentCmd)
...
func lockUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.LockUnspentCmd)
...
// and amounts.
func makeOutputs(pairs map[string]acmutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
// All errors are returned in acmjson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]acmutil.Amount,
	account uint32, minconf int32, feeSatPerKb acmutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := acmutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]acmutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]acmutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := acmutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func sendToAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.SendToAddressCmd)
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.SignMessageCmd)
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*acmjson.SignRawTransactionCmd)
	...
	// TODO: really we probably should look these up with acmd anyway to
	// make sure that they match the blockchain if present.
	inputs := make(map[wire.OutPoint][]byte)
	scripts := make(map[string][]byte)
	var cmdInputs []acmjson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := acmutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	// querying acmd with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*acmutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*acmutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := acmutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := acmutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	signErrors := make([]acmjson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, acmjson.SignRawTransactionError{
	...
	return acmjson.SignRawTransactionResult{
...
func validateAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.ValidateAddressCmd)

	result := acmjson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.VerifyMessageCmd)
	...
	case *acmutil.AddressPubKeyHash: // ok
		return bytes.Equal(acmutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *acmutil.AddressPubKey: // ok
...
func walletPassphrase(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.WalletPassphraseCmd)
...
func walletPassphraseChange(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*acmjson.WalletPassphraseChangeCmd)
