...
//
//   https://github.com/Actinium-project/acmwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/Actinium-project/acmwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"github.com/Actinium-project/acmwallet/internal/zero"
	"github.com/Actinium-project/acmwallet/netparams"
	pb "github.com/Actinium-project/acmwallet/rpc/walletrpc"
...
// as well as establishing a RPC connection to a acmd consensus server.
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := acmutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	var totalAmount acmutil.Amount
	for _, output := range unspentOutputs {
		...
		totalAmount += acmutil.Amount(output.Output.Value)

		if req.TargetAmount != 0 && totalAmount > acmutil.Amount(req.TargetAmount) {
			break
		}
	}

	var changeScript []byte
	if req.IncludeChangeScript && totalAmount > acmutil.Amount(req.TargetAmount) {
...
//   sendrawtransaction, so connection errors to acmd could result in the tx
