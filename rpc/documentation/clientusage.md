...
var certificateFile = filepath.Join(acmutil.AppDataDir("acmwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", acmutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/acmwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/acmwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by acmwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.acmwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\acmwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by acmwallet).
...
var certPath = path.join(process.env.HOME, '.acmwallet', 'rpc.cert');
...
```bash
$ protoc -I /path/to/Actinium-project/acmwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.acmwallet', 'rpc.cert')
