...
// Address returns the acmutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() acmutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*acmutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return acmutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	if compressed {
		...
		pubKeyHash = acmutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address acmutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := acmutil.NewAddressWitnessPubKeyHash(
		...
		address, err = acmutil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = acmutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *acmutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() acmutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := acmutil.NewAddressScriptHashFromHash(
