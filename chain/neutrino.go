...
func NewNeutrinoClient(chainParams *chaincfg.Params,
	chainService *actrino.ChainService) *NeutrinoClient {
...
// pollCFilter attempts to fetch a CFilter from the actrino client. This is
...
func (s *NeutrinoClient) Rescan(startHash *chainhash.Hash, addrs []acmutil.Address,
	outPoints map[wire.OutPoint]acmutil.Address) error {
	...
	var inputsToWatch []actrino.InputWithScript
	for op, addr := range outPoints {
		...
		inputsToWatch = append(inputsToWatch, actrino.InputWithScript{
			OutPoint: op,
			PkScript: addrScript,
		})
	}
	...
	s.clientMtx.Lock()
	newRescan := actrino.NewRescan(
		&actrino.RescanChainSource{
			ChainService: s.CS,
		},
		actrino.NotificationHandlers(rpcclient.NotificationHandlers{
			OnBlockConnected:         s.onBlockConnected,
			OnFilteredBlockConnected: s.onFilteredBlockConnected,
			OnBlockDisconnected:      s.onBlockDisconnected,
		}),
		actrino.StartBlock(&headerfs.BlockStamp{Hash: *startHash}),
		actrino.StartTime(s.startTime),
		actrino.QuitChan(s.rescanQuit),
		actrino.WatchAddrs(addrs...),
		actrino.WatchInputs(inputsToWatch...),
	)
	s.rescan = newRescan
...
func (s *NeutrinoClient) NotifyBlocks() error {
	...
	if !s.scanning {
		s.clientMtx.Unlock()
		return s.NotifyReceived([]acmutil.Address{})
	}
	s.clientMtx.Unlock()
	return nil
}
...
func (s *NeutrinoClient) NotifyReceived(addrs []acmutil.Address) error {
	s.clientMtx.Lock()
	...
	if s.scanning {
		s.clientMtx.Unlock()
		return s.rescan.Update(actrino.AddAddrs(addrs...))
	}
...
func (s *NeutrinoClient) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, relevantTxs []*acmutil.Tx) {
