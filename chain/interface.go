...
// acmd RPC chain server, or an SPV library, as long as we write a driver for
// it.
type Interface interface {
	...
	Rescan(*chainhash.Hash, []acmutil.Address, map[wire.OutPoint]acmutil.Address) error
	NotifyReceived([]acmutil.Address) error
	NotifyBlocks() error
...
type (
	...
	FilterBlocksRequest struct {
		...
		ExternalAddrs    map[waddrmgr.ScopedIndex]acmutil.Address
		InternalAddrs    map[waddrmgr.ScopedIndex]acmutil.Address
	...
	FilterBlocksResponse struct {
		...
		FoundOutPoints     map[wire.OutPoint]acmutil.Address
