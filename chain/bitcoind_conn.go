...
type BitcoindConn struct {
	...
	// each new actiniumd rescan client using the current actiniumd
	// connection.
	rescanClientCounter uint64

	// chainParams identifies the current network the actiniumd node is
	// running on.
	chainParams *chaincfg.Params

	// client is the RPC client to the actiniumd node.
	...
	// rescanClients is the set of active actiniumd rescan clients to which
...
// Start attempts to establish a RPC and ZMQ connection to a actiniumd node. If
...
// Stop terminates the RPC and ZMQ connection to a actiniumd node and removes any
...
func (c *BitcoindConn) blockEventHandler() {
	...
	log.Info("Started listening for Actiniumd block notifications via ZMQ "+
	...
	for {
		...
		default:
			...
			// actiniumd shuts down, which will produce an unreadable
...
func (c *BitcoindConn) txEventHandler() {
	...
	log.Info("Started listening for Actiniumd transaction notifications "+
...
// getCurrentNet returns the network on which the actiniumd node is running.
...
// NewBitcoindClient returns a actiniumd client using the current actiniumd
