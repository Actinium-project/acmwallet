...
func (c *BitcoindClient) GetBlockVerbose(
	hash *chainhash.Hash) (*acmjson.GetBlockVerboseResult, error) {
...
func (c *BitcoindClient) GetBlockHeaderVerbose(
	hash *chainhash.Hash) (*acmjson.GetBlockHeaderVerboseResult, error) {
...
func (c *BitcoindClient) GetRawTransactionVerbose(
	hash *chainhash.Hash) (*acmjson.TxRawResult, error) {
...
func (c *BitcoindClient) GetTxOut(txHash *chainhash.Hash, index uint32,
	mempool bool) (*acmjson.GetTxOutResult, error) {
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []acmutil.Address) error {
	c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]acmutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []acmutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]acmutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
// matched as []acmjson.BlockDetails.
func (c *BitcoindClient) RescanBlocks(
	blockHashes []chainhash.Hash) ([]acmjson.RescannedBlock, error) {

	rescannedBlocks := make([]acmjson.RescannedBlock, 0, len(blockHashes))
	for _, hash := range blockHashes {
		...
		if len(relevantTxs) > 0 {
			rescannedBlock := acmjson.RescannedBlock{
				Hash: hash.String(),
			}
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []acmutil.Address, outPoints map[wire.OutPoint]acmutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]acmutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
...
func (c *BitcoindClient) rescan(start chainhash.Hash) error {
	...
	for i := previousHeader.Height + 1; i <= bestBlock.Height; i++ {
			...
			if headers.Back() != nil {
				...
				if headers.Back() != nil {
					previousHeader = headers.Back().
						Value.(*acmjson.GetBlockHeaderVerboseResult)
		...
		previousHeader = &acmjson.GetBlockHeaderVerboseResult{
