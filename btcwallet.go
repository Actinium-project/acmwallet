...
func rpcClientConnectLoop(legacyRPCServer *legacyrpc.Server, loader *wallet.Loader) {
	...
	for {
		...
		if cfg.UseSPV {
			var (
				chainService *actrino.ChainService
				spvdb        walletdb.DB
			)
			netDir := networkDir(cfg.AppDataDir.Value, activeNet.Params)
			spvdb, err = walletdb.Create("bdb",
				filepath.Join(netDir, "actrino.db"))
			defer spvdb.Close()
			if err != nil {
				log.Errorf("Unable to create Neutrino DB: %s", err)
				continue
			}
			chainService, err = actrino.NewChainService(
				actrino.Config{
					DataDir:      netDir,
					Database:     spvdb,
...
// startChainRPC opens a RPC client connection to a acmd server for blockchain
