...
import (
	...
	"github.com/Actinium-project/acmwallet/internal/cfgutil"
	"github.com/Actinium-project/acmwallet/internal/legacy/keystore"
	"github.com/Actinium-project/acmwallet/netparams"
...
const (
	defaultCAFilename       = "acmd.cert"
	defaultConfigFilename   = "acmwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "acmwallet.log"
...
var (
	acmdDefaultCAFile  = filepath.Join(acmutil.AppDataDir("acmd", false), "rpc.cert")
	defaultAppDataDir  = acmutil.AppDataDir("acmwallet", false)
...
type config struct {
	...
	TestNet4      bool                    `long:"testnet" description:"Use the test Actinium network (version 4) (default mainnet)"`
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of acmd RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with acmd"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	BtcdUsername     string                  `long:"acmdusername" description:"Username for acmd authentication"`
	BtcdPassword     string                  `long:"acmdpassword" default-mask:"-" description:"Password for acmd authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and acmd authentication (if acmdusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and acmd authentication (if acmdpassword is unset)"`
...
//
// The above results in acmwallet functioning properly without any config
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		AddPeers:               []string{},
		ConnectPeers:           []string{},
		MaxPeers:               actrino.MaxPeers,
		BanDuration:            actrino.BanDuration,
		BanThreshold:           actrino.BanThreshold,
	}
	...
	if cfg.UseSPV {
		actrino.MaxPeers = cfg.MaxPeers
		actrino.BanDuration = cfg.BanDuration
		actrino.BanThreshold = cfg.BanThreshold
	} else {
		if cfg.RPCConnect == "" {
		...
		if cfg.DisableClientTLS {
			...
			// If CAFile is unset, choose either the copy or local acmd cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local acmd and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						acmdCertExists, err := cfgutil.FileExists(
							acmdDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						...
						if acmdCertExists {
							cfg.CAFile.Value = acmdDefaultCAFile
						}
					}
				}
			}
	...
	// If the acmd username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for acmd and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for acmd and wallet.
	if cfg.BtcdUsername == "" {
