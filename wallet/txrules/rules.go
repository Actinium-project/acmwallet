...
const DefaultRelayFeePerKb acmutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb acmutil.Amount) acmutil.Amount {
	...
	relayFee := acmutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount acmutil.Amount, scriptSize int, relayFeePerKb acmutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb acmutil.Amount) bool {
	...
	return IsDustAmount(acmutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb acmutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > acmutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb acmutil.Amount, txSerializeSize int) acmutil.Amount {
	fee := relayFeePerKb * acmutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > acmutil.MaxSatoshi {
		fee = acmutil.MaxSatoshi
	}
