...
type RecoveryState struct {
	...
	// a rescan.
	watchedOutPoints map[wire.OutPoint]acmutil.Address
}
...
func NewRecoveryState(recoveryWindow uint32) *RecoveryState {
	...
	return &RecoveryState{
		recoveryWindow:   recoveryWindow,
		scopes:           scopes,
		watchedOutPoints: make(map[wire.OutPoint]acmutil.Address),
	}
}
...
func (rs *RecoveryState) WatchedOutPoints() map[wire.OutPoint]acmutil.Address {
...
type BranchRecoveryState struct {
	...
	addresses map[uint32]acmutil.Address
...
func NewBranchRecoveryState(recoveryWindow uint32) *BranchRecoveryState {
	return &BranchRecoveryState{
		recoveryWindow:  recoveryWindow,
		addresses:       make(map[uint32]acmutil.Address),
...
func (brs *BranchRecoveryState) AddAddr(index uint32, addr acmutil.Address) {
	brs.addresses[index] = addr
}

// GetAddr returns the address derived from a given child index.
func (brs *BranchRecoveryState) GetAddr(index uint32) acmutil.Address {
...
func (brs *BranchRecoveryState) Addrs() map[uint32]acmutil.Address {
