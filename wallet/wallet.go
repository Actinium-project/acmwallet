...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]acmutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	var addrs []acmutil.Address
	err := w.Manager.ForEachActiveAddress(addrmgrNs, func(addr acmutil.Address) error {
...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]acmutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]acmutil.Address),
...
func (w *Wallet) CreateSimpleTx(account uint32, outputs []*wire.TxOut,
	minconf int32, satPerKb acmutil.Amount, dryRun bool) (
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []acmutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (acmutil.Amount, error) {
...
type Balances struct {
	Total          acmutil.Amount
	Spendable      acmutil.Amount
	ImmatureReward acmutil.Amount
}
...
// blockchain or acmd mempool), the next chained address is returned.
func (w *Wallet) CurrentAddress(account uint32, scope waddrmgr.KeyScope) (acmutil.Address, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return nil, err
	}
	...
	if props != nil {
		err = chainClient.NotifyReceived([]acmutil.Address{addr})
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) PubKeyForAddress(a acmutil.Address) (*btcec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a acmutil.Address) (*btcec.PrivateKey, error) {
...
func (w *Wallet) HaveAddress(a acmutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a acmutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a acmutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	syncHeight int32, net *chaincfg.Params) []acmjson.ListTransactionsResult {
	...
	results := []acmjson.ListTransactionsResult{}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := acmutil.Amount(output.Value).ToBTC()
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]acmjson.ListTransactionsResult, error) {
	txList := []acmjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]acmjson.ListTransactionsResult, error) {
	txList := []acmjson.ListTransactionsResult{}
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]acmjson.ListTransactionsResult, error) {
	txList := []acmjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*acmutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]acmjson.ListTransactionsResult, error) {
	txList := []acmjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*acmutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32,
	addresses map[string]struct{}) ([]*acmjson.ListUnspentResult, error) {

	var results []*acmjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		results = make([]*acmjson.ListUnspentResult, 0, len(unspent))
		for i := range unspent {
			output := unspent[i]
		...
		scSwitch:
			...
			result := &acmjson.ListUnspentResult{
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr acmutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr acmutil.Address) (string, error) {
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *acmutil.WIF,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]acmutil.Address{addr})
		if err != nil {
...
func (w *Wallet) LockedOutpoints() []acmjson.TransactionInput {
	locked := make([]acmjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = acmjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (acmutil.Address, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (acmutil.Address, *waddrmgr.AccountProperties, error) {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32) (acmutil.Address, error) {
...
// address.
func (w *Wallet) TotalReceivedForAddr(addr acmutil.Address, minConf int32) (acmutil.Amount, error) {
	var amount acmutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb acmutil.Amount) (*wire.MsgTx, error) {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*acmutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr acmutil.Address) (*btcec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr acmutil.Address) ([]byte, error) {
...
func (w *Wallet) publishTransaction(tx *wire.MsgTx) (*chainhash.Hash, error) {
	...
	if rpcErr, ok := err.(*acmjson.RPCError); ok {
		rpcTxConfirmed = rpcErr.Code == acmjson.ErrRPCTxAlreadyInChain
	}
...
// in order to allow applications wrapping acmwallet to store app-specific data
