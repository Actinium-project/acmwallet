...
func (w *Wallet) MakeMultiSigScript(addrs []acmutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*acmutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*acmutil.AddressScriptHash, error) {
	var p2shAddr *acmutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = acmutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*acmutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
